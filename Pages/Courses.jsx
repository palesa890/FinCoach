...
export default function Courses() {
  ...
  const handleCourseComplete = async (course) => {
    try {
      ...
      if (existing) {
        await CourseProgress.update(existing.id, { completed: true, progress: 100 });
      } else {
  ...
  const getCourseProgress = (courseUrl) => {
    return courseProgress.find(p => p.course_url === courseUrl);
  };
  ...
  const filteredCourses = courses.filter(course => {
    ...
    const matchesPricing = selectedPricing === "all" || (selectedPricing === "free" ? course.is_free : !course.is_free);
    return matchesSearch && matchesCategory && matchesDifficulty && matchesPricing;
  });

  const categories = ["all", "budgeting", "investing", "debt_management", "credit_building", "student_finance", "behavioral_finance"];
  const difficulties = ["all", "beginner", "intermediate", "advanced"];

  const categoryColors = {
    budgeting: "bg-emerald-100 text-emerald-800",
    investing: "bg-blue-100 text-blue-800",
    debt_management: "bg-amber-100 text-amber-800",
    credit_building: "bg-purple-100 text-purple-800",
    student_finance: "bg-pink-100 text-pink-800",
    behavioral_finance: "bg-indigo-100 text-indigo-800"
  };

  if (selectedCourse) {
    const progress = getCourseProgress(selectedCourse.url);
    return (
      <CourseDetail 
        course={selectedCourse}
        progress={progress}
        onBack={() => setSelectedCourse(null)}
        onStart={() => handleCourseStart(selectedCourse)}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-lg text-slate-600">
                Discover courses to boost your financial skills
              </p>
            </div>
            <div className="w-full md:w-auto">
              <UserProgress courseProgress={courseProgress} isLoading={isLoading} />
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  <select
                    ...
                    className="px-3 py-2 border border-slate-200 rounded-md bg-white text-sm w-full"
                  >
                    {categories.map(cat => (
